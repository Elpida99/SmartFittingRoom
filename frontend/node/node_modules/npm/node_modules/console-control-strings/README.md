...
doing things like colour and cursor positioning.  This is a subset of both
...
console.log(consoleControl.colour('blue','bgRed', 'bold') + 'hi there' + consoleControl.colour('reset'))
...
2. Could emit, for instance, a start colour code without an end one.
...
### var code = consoleControl.colour(_colors = []_)

### var code = consoleControl.colour(_color1_, _color2_, _…_, _colorn_)

Returns the escape sequence to set the current terminal display attributes
(mostly colors).  Arguments can either be a list of attributes or an array
of attributes.  The difference between passing in an array or list of colors
and calling `.colour` separately for each one, is that in the former case a
...
* Styles:
  * **bold** – Display text as bold.  In some terminals this means using a
    bold font, in others this means changing the colour.  In some it means
    both.
