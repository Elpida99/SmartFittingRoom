...
* Reset colors prior to ending a line, to eliminate flicker when a line
  is trucated between start and end colour sequences.
...
* Fix bug where if a theme name were given instead of a theme object, it
  would crash.
* Remove supports-colour because it's not actually used.  Uhm.  Yes, I just
  updated it.  >.>

### v2.7.2

* Use supports-colour instead of has-colour (as the module has been renamed)
...
* Use our own colour support detection.
  The `has-colour` module proved too magic for my needs, making assumptions
...
* Themes are entirely different.  If you were using a custom theme, or
  ...
  var unicodeTheme = themes(true, true) // returns the colour unicode theme for your platform
  ```
